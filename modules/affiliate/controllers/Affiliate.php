...
class Affiliate extends Public_Controller
{
	...
	{
		parent::__construct(); 
		$this->load->model(array('affiliate/affiliate_model','payment/payment_model','order/order_model'));
	...
	{	
		...
		// $data['heading_title'] = "Advanced Search";						
		 $this->load->view('affiliate/view_affiliate_register_student');
	}
	...
	//  }
	 public function register()
	 {
		...
		switch($id)
		{
			...
			break;
			case "teacher":
				$this->load->view('affiliate/view_affiliate_register_teacher');
			break;
			default:
			echo "<script>alert('Error Occured. Please Try Again'); window.location = '".base_url()."'</script>";
	 ...
	 public function studentRegister()
	 {
		 ...
		 //if($category == "")
		 //{
			//echo "<script>alert('Please Select a affiliate'); window.location = '".base_url()."affiliate/register/student'</script>";
		// }
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			if($jsonSignup['Result']['msg'] == "Item has been added")
			{
				...
				//{
				//$sqq = "INSERT INTO `wl_affiliate_register` (`emailid`, `affiliate_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '1', '2','".$ip."')";
					//echo $sqq;
				//$que = $dbe->query($sqq); 
				//}
			...
			{
				echo "<script>alert('Error Occured. Please Try Again'); window.location = '".base_url()."affiliate/register/student'</script>";
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			for($i=0;$i<count($category);$i++)
			{
			$sqq = "INSERT INTO `wl_affiliate_register` (`emailid`, `affiliate_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '0', '2','".$ip."')";
			$que = $dbe->query($sqq); 
			}
			echo "<script>alert('Registered Successfully!'); window.location = '".base_url()."'</script>";
...
public function parentRegister()
	 {
		 ...
		 if($category == "")
		 {
			echo "<script>alert('Please Select a affiliate'); window.location = '".base_url()."affiliate/register/parent'</script>";
		 }
		$category_str = implode(",",$category);
		//print_r($category);
		$password = $this->input->post('password',TRUE);
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			if($jsonSignup['Result']['msg'] == "Item has been added")
			{
				...
				for($i=0;$i<count($category);$i++)
				{
				$sqq = "INSERT INTO `wl_affiliate_register` (`emailid`, `affiliate_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '1', '1','".$ip."')";
					//echo $sqq;
				$que = $dbe->query($sqq); 
				}
				echo "<script>alert('Registered Successfully!'); window.location = '".base_url()."'</script>";
			...
			{
				echo "<script>alert('Error Occured. Please Try Again'); window.location = '".base_url()."affiliate/register/parent'</script>";
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			for($i=0;$i<count($category);$i++)
			{
			$sqq = "INSERT INTO `wl_affiliate_register` (`emailid`, `affiliate_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '0', '1','".$ip."')";
			$que = $dbe->query($sqq); 
			}
			echo "<script>alert('Registered Successfully!'); window.location = '".base_url()."'</script>";
	 ...
	 public function teacherRegister()
	 {
		// $category = $this->input->post('category',TRUE);
		 //if($category == "")
		 //{
		//	echo "<script>alert('Please Select a affiliate'); window.location = '".base_url()."affiliate/register/teacher'</script>";
		 //}
		//$category_str = implode(",",$category);
		//print_r($category);
		$password = $this->input->post('password',TRUE);
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			$chh = curl_init();  
			$nameUrl = str_replace(" ", "%20", $name);
			$urll = "https://www.pathshala.co/decore/new/api.php?action=TeacherRegistration&email=".$email."&pass=".$password."&name=".$nameUrl."&phone=".$phone."&referral_code=".$referralcode;
			curl_setopt($chh,CURLOPT_URL,$urll);
			curl_setopt($chh,CURLOPT_RETURNTRANSFER,true);
			$outputt=curl_exec($chh);
			curl_close($chh);
			...
			//	for($i=0;$i<count($category);$i++)
			//	{
			//	$sqq = "INSERT INTO `wl_affiliate_register` (`emailid`, `affiliate_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '1', '3','".$ip."')";
			//		//echo $sqq;
			//	$que = $dbe->query($sqq); 
			//	}
				echo "<script>alert('Registered Successfully!'); window.location = '".base_url()."'</script>";
			...
			{
				echo "<script>alert('Error Occured. Please Try Again'); window.location = '".base_url()."affiliate/register/teacher'</script>";
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			for($i=0;$i<count($category);$i++)
			{
			$sqq = "INSERT INTO `wl_affiliate_register` (`emailid`, `affiliate_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '0', '3','".$ip."')";
			$que = $dbe->query($sqq); 
			}
			echo "<script>alert('Registered Successfully!'); window.location = '".base_url()."'</script>";
