<?php
class Freegift extends Public_Controller
{
	public function __construct()
	{
		parent::__construct(); 
		$this->load->model(array('freegift/freegift_model','payment/payment_model','order/order_model'));
	...
	{	
        ...
        $config['base_url']    = base_url().'freegift/ajaxPaginationData';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $this->perPage;
        $config['link_func']   = 'searchFilter';
        $this->ajax_pagination->initialize($config);
        //get the posts data
        $data['res'] = $this->freegift_model->get_course_row(array('limit'=>$this->perPage));
        $data['totalRec']=$totalRec;
        //load the view
		$data['heading_title'] = "Advanced Search";						
		 $this->load->view('freegift/view_freegift',$data);
	}
	...
	public  function ajaxPaginationData(){
        ...
        //get posts data
        $data['res'] = $this->freegift_model->get_course_row($conditions);
        //echo_sql();
        //load the view
        $this->load->view('freegift/view_search_freegift', $data, false);
		}
...
/*payment gateway paytm integration END*/	
	...
	public function payment_status_paytm(){
	 ...
	 {
		$this->load->view('freegift/view_freegift');
	 }
	 public function speakerregister()
	 {
		$this->load->view('freegift/view_freegift_register_teacher');
	 }
	 public function freegiftRegister()
	 {
		$password = $this->input->post('password',TRUE);
		$email = $this->input->post('user_name',TRUE);
		$name = $this->input->post('first_name',TRUE);
		$phone = $this->input->post('phone_number',TRUE);
		$freegift_id = $this->input->post('freegift',TRUE);
		if($freegift_id == "")
		{
		   echo "<script>alert('Please Select a Category from Dropdown'); window.location = '".base_url()."freegift/register'</script>";
		}
		$freegift_str = implode(",",$freegift_id);
		$ch = curl_init();  
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			{
				echo "<script>alert('Error Occured. Please Try Again'); window.location = '".base_url()."freegift'</script>";	
			}
			else
			{
				$dbe = $this->load->database('default', TRUE);
				$ip=$_SERVER['REMOTE_ADDR'];
				for($i=0;$i<count($freegift_id);$i++)
				{
					$sqq = "INSERT INTO `wl_freegift_register` (`emailid`, `freegift_id`, `ip`, `user_type`) values ('".$email."','".$freegift_id[$i]."','".$ip."', '1')";
					$que = $dbe->query($sqq); 
				}
				// $sqq = "INSERT INTO `wl_freegift_register` (`emailid`, `freegift_id`, `ip`, `user_type`) values ('".$email."','".$freegift_id."','".$ip."', '1')";
				// $que = $dbe->query($sqq); 
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			for($i=0;$i<count($freegift_id);$i++)
				{
					$sqq = "INSERT INTO `wl_freegift_register` (`emailid`, `freegift_id`, `ip`) values ('".$email."','".$freegift_id[$i]."','".$ip."')";
					$que = $dbe->query($sqq); 
				}
			// $sqq = "INSERT INTO `wl_freegift_register` (`emailid`, `freegift_id`, `ip`) values ('".$email."','".$freegift_id."','".$ip."')";
			// $que = $dbe->query($sqq); 
			echo "<script>alert('Registered Successfully. Proceed With Login'); window.location = '".base_url()."users/login'</script>";
	 ...
	 //TeacherRegistration
	 public function freegiftRegisterTeacher()
	 {
		$password = $this->input->post('password',TRUE);
		$email = $this->input->post('user_name',TRUE);
		$name = $this->input->post('first_name',TRUE);
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			else
			{
				$dbe = $this->load->database('default', TRUE);
				$ip=$_SERVER['REMOTE_ADDR'];
				$sqq = "INSERT INTO `wl_freegift_Teacher` (`emailid`, `ip`, `user_type`) values ('".$email."','".$ip."', '1')";
				$que = $dbe->query($sqq); 
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			$dbe = $this->load->database('default', TRUE);
			$ip=$_SERVER['REMOTE_ADDR'];
			$sqq = "INSERT INTO `wl_freegift_Teacher` (`emailid`, `ip`) values ('".$email."','".$ip."')";
			$que = $dbe->query($sqq); 
			echo "<script>alert('Registered Successfully. Proceed With Login'); window.location = '".base_url()."teacher/login'</script>";
	...
	public function studentRegister()
	 {
		 ...
		 //if($category == "")
		 //{
			//echo "<script>alert('Please Select a freegift'); window.location = '".base_url()."freegift/register/student'</script>";
		// }
		...
		$name = $this->input->post('first_name',TRUE);
		$phone = $this->input->post('phone_number',TRUE);
		$referralcode = $this->input->post('referralcode',TRUE);
		$ch = curl_init();  
		$url = "https://www.pathshala.co/decore/new/api.php?action=Login&userName=".$email."&pass=nullpass";
		curl_setopt($ch,CURLOPT_URL,$url);
		curl_setopt($ch,CURLOPT_RETURNTRANSFER,true);
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			$chh = curl_init();  
			$nameUrl = str_replace(" ", "%20", $name);
			$urll = "https://www.pathshala.co/decore/new/api.php?action=registerUser&email=".$email."&pass=".$password."&name=".$nameUrl."&phone=".$phone."&referral_code=".$referralcode;
			curl_setopt($chh,CURLOPT_URL,$urll);
			curl_setopt($chh,CURLOPT_RETURNTRANSFER,true);
			$outputt=curl_exec($chh);
			curl_close($chh);
			...
			if($jsonSignup['Result']['msg'] == "Item has been added")
			{
				...
				//{
				//$sqq = "INSERT INTO `wl_freegift_register` (`emailid`, `freegift_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '1', '2','".$ip."')";
					//echo $sqq;
				//$que = $dbe->query($sqq); 
				//}
				echo "<script>alert('Registered Successfully!'); window.location = '".base_url()."thankyou'</script>";
			}
			else
			{
				echo "<script>alert('Error Occured. Please Try Again'); window.location = '".base_url()."freegift'</script>";
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			for($i=0;$i<count($category);$i++)
			{
			$sqq = "INSERT INTO `wl_freegift_register` (`emailid`, `freegift_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '0', '2','".$ip."')";
			$que = $dbe->query($sqq); 
			}
