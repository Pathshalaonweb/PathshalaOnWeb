...
class Test extends Public_Controller
{
	...
	{
		parent::__construct(); 
		$this->load->model(array('test/test_model','payment/payment_model','order/order_model'));
	...
	{	
 		...
 		$data = array();
        //total rows count
        $totalRec = count($this->test_model->get_course_row());
        //pagination configuration
        $config['target']      = '#postList';
        $config['base_url']    = base_url().'test/ajaxPaginationData';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $this->perPage;
        $config['link_func']   = 'searchFilter';
        $this->ajax_pagination->initialize($config);
        //get the posts data
        $data['res'] = $this->test_model->get_course_row(array('limit'=>$this->perPage));
        $data['totalRec']=$totalRec;
        //load the view
		$data['heading_title'] = "Advanced Search";						
		 $this->load->view('test/view_test',$data);
	}
	
	public function detail(){
		$id=$this->uri->segment('3');
		$param=array('courses_friendly_url'=>$id);
		$res = $this->test_model->get_courses(1,0,$param);
		$data['res']=$res;
		$this->load->view('test/view_course_detail',$data);
	}
	...
	public  function ajaxPaginationData(){
		...
		//total rows count
        $totalRec = count($this->test_model->get_course_row($conditions));
        ...
        //get posts data
        $data['res'] = $this->test_model->get_course_row($conditions);
        //echo_sql();
        //load the view
        $this->load->view('test/view_search_courses', $data, false);
		}
		...
		public function enrollDetailStoreCredit(){
			...
			$this->session->set_userdata($newdata);	
			redirect('test/enrollStoreCredit', 'refresh');
		}
		
		public function enrollStoreCredit(){
			$course_id 		= $this->session->userdata('course_id');
			 $param 			= array('status'=>'1','course_id'=>$course_id);	
			 $res_array			=$this->test_model->get_courses(1,0,$param);
			// print_r($res_array);
			 $data['res'] 		= $res_array;
			 $this->load->view('test/view_course_enroll_StoreCredit', $data, false);	
		}
		...
		public function enroll(){
			...
			// print_r($res_array);
			 $data['res'] 		= $res_array;
			 $this->load->view('test/view_course_enroll', $data, false);	
		}
		...
		public function payment(){
			...
			if ($this->input->post('pay_method') == "paytm" ) {
					...
					$data = array(
							...
							'return_url'	    =>  base_url() . 'test/payment_status_paytm',
...
/*payment gateway paytm integration END*/	
 ...
 {
	$this->load->view('test/view_test_register');
 }
 public function webinarRegister()
	 {
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			{
				echo "<script>alert('Error Occured. Please Try Again'); window.location = '".base_url()."test'</script>";	
			}
			else
			{
