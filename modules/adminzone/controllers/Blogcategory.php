	...
	{	
	 ...
	 public  function index()
	 {
		...
		public function add() {			
	        ...
	        $this->form_validation->set_rules('name','Category',"trim|required|max_length[255]|xss_clean|unique[wl_blogcategory.name='".$this->db->escape_str($this->input->post('name'))."' AND status!='2']");
			
			$this->form_validation->set_rules('url','Link','trim|required|required_stripped|xss_clean|max_length[8500]');
			
			if($this->form_validation->run()==TRUE)
			{
			
			      $posted_data = array(
				  	'blogcategory_id' => $this->input->post('blogcategory_id',TRUE),
	   ...
	   public function edit()
	   {
		    ...
		    $data['heading_title'] = 'Edit Blog Category';
		   ...
		   if(is_array($res) && !empty($res)){ 
		        $this->form_validation->set_rules('name','Category',"trim|required|max_length[255]|xss_clean|unique[wl_blogcategory.name='".$this->db->escape_str($this->input->post('name'))."' AND status!='2' AND blogcategory_id!=".$Id."]");
