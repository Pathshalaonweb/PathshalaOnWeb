...
function paypalForm($order_res)
{
	...
	$total_tax=(($order_res['total_amount']*$order_res['tax_amount'])/100);
	
	
	$amount    = (($order_res['total_amount']) + ( $order_res['shipping_amount']) - ( $order_res['coupon_discount_amount']) + ($total_tax));
...
<body style="margin:0px;">    
    <div style="text-align:left; font-size: 22px; font-weight:bold;  background-color:#f9f9f9; border:#efefef; padding:20px;">
		...
		<input type="hidden" name="business" value="amit.perfectinfotech@gmail.com"> 
