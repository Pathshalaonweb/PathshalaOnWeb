...
class Thankyou extends Public_Controller
{
	...
	{
		parent::__construct(); 
		$this->load->model(array('thankyou/thankyou_model','payment/payment_model','order/order_model'));
	...
	{	
        ...
        $config['base_url']    = base_url().'thankyou/ajaxPaginationData';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $this->perPage;
        $config['link_func']   = 'searchFilter';
        $this->ajax_pagination->initialize($config);
        //get the posts data
        $data['res'] = $this->thankyou_model->get_course_row(array('limit'=>$this->perPage));
        $data['totalRec']=$totalRec;
        //load the view
		$data['heading_title'] = "Advanced Search";						
		 $this->load->view('thankyou/view_thankyou',$data);
	}
	...
	public  function ajaxPaginationData(){
        ...
        //get posts data
        $data['res'] = $this->thankyou_model->get_course_row($conditions);
        //echo_sql();
        //load the view
        $this->load->view('thankyou/view_search_thankyou', $data, false);
		}
...
/*payment gateway paytm integration END*/	
	...
	public function payment_status_paytm(){
	 ...
	 {
		$this->load->view('thankyou/view_thankyou');
	 }
	 public function speakerregister()
	 {
		$this->load->view('thankyou/view_thankyou_register_teacher');
	 }
	 public function thankyouRegister()
	 {
		$password = $this->input->post('password',TRUE);
		$email = $this->input->post('user_name',TRUE);
		$name = $this->input->post('first_name',TRUE);
		$phone = $this->input->post('phone_number',TRUE);
		$thankyou_id = $this->input->post('thankyou',TRUE);
		if($thankyou_id == "")
		{
		   echo "<script>alert('Please Select a Category from Dropdown'); window.location = '".base_url()."thankyou/register'</script>";
		}
		$thankyou_str = implode(",",$thankyou_id);
		$ch = curl_init();  
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			{
				echo "<script>alert('Error Occured. Please Try Again'); window.location = '".base_url()."thankyou'</script>";	
			}
			else
			{
				$dbe = $this->load->database('default', TRUE);
				$ip=$_SERVER['REMOTE_ADDR'];
				for($i=0;$i<count($thankyou_id);$i++)
				{
					$sqq = "INSERT INTO `wl_thankyou_register` (`emailid`, `thankyou_id`, `ip`, `user_type`) values ('".$email."','".$thankyou_id[$i]."','".$ip."', '1')";
					$que = $dbe->query($sqq); 
				}
				// $sqq = "INSERT INTO `wl_thankyou_register` (`emailid`, `thankyou_id`, `ip`, `user_type`) values ('".$email."','".$thankyou_id."','".$ip."', '1')";
				// $que = $dbe->query($sqq); 
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			for($i=0;$i<count($thankyou_id);$i++)
				{
					$sqq = "INSERT INTO `wl_thankyou_register` (`emailid`, `thankyou_id`, `ip`) values ('".$email."','".$thankyou_id[$i]."','".$ip."')";
					$que = $dbe->query($sqq); 
				}
			// $sqq = "INSERT INTO `wl_thankyou_register` (`emailid`, `thankyou_id`, `ip`) values ('".$email."','".$thankyou_id."','".$ip."')";
			// $que = $dbe->query($sqq); 
			echo "<script>alert('Registered Successfully. Proceed With Login'); window.location = '".base_url()."users/login'</script>";
	 ...
	 //TeacherRegistration
	 public function thankyouRegisterTeacher()
	 {
		$password = $this->input->post('password',TRUE);
		$email = $this->input->post('user_name',TRUE);
		$name = $this->input->post('first_name',TRUE);
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			else
			{
				$dbe = $this->load->database('default', TRUE);
				$ip=$_SERVER['REMOTE_ADDR'];
				$sqq = "INSERT INTO `wl_thankyou_Teacher` (`emailid`, `ip`, `user_type`) values ('".$email."','".$ip."', '1')";
				$que = $dbe->query($sqq); 
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			$dbe = $this->load->database('default', TRUE);
			$ip=$_SERVER['REMOTE_ADDR'];
			$sqq = "INSERT INTO `wl_thankyou_Teacher` (`emailid`, `ip`) values ('".$email."','".$ip."')";
			$que = $dbe->query($sqq); 
			echo "<script>alert('Registered Successfully. Proceed With Login'); window.location = '".base_url()."teacher/login'</script>";
	...
	public function studentRegister()
	 {
		 ...
		 //if($category == "")
		 //{
			//echo "<script>alert('Please Select a thankyou'); window.location = '".base_url()."thankyou/register/student'</script>";
		// }
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			if($jsonSignup['Result']['msg'] == "Item has been added")
			{
				...
				//{
				//$sqq = "INSERT INTO `wl_thankyou_register` (`emailid`, `thankyou_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '1', '2','".$ip."')";
					//echo $sqq;
				//$que = $dbe->query($sqq); 
				//}
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			for($i=0;$i<count($category);$i++)
			{
			$sqq = "INSERT INTO `wl_thankyou_register` (`emailid`, `thankyou_enrolled`, `user_type`, `user_category`, `ip`) values ('".$email."','".$category[$i]."', '0', '2','".$ip."')";
			$que = $dbe->query($sqq); 
			}
			echo "<script>alert('Registered Successfully!'); window.location = '".base_url()."'</script>";
