<?php
class Newtest extends Public_Controller
{
	public function __construct()
	{
		parent::__construct(); 
		$this->load->model(array('newtest/newtest_model','payment/payment_model','order/order_model'));
	...
	{	
        ...
        $config['base_url']    = base_url().'newtest/ajaxPaginationData';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $this->perPage;
        $config['link_func']   = 'searchFilter';
        $this->ajax_pagination->initialize($config);
        //get the posts data
        $data['res'] = $this->newtest_model->get_course_row(array('limit'=>$this->perPage));
        $data['totalRec']=$totalRec;
        //load the view
		$data['heading_title'] = "Advanced Search";						
		 $this->load->view('newtest/view_newtest',$data);
	}
	...
	public  function ajaxPaginationData(){
        ...
        //get posts data
        $data['res'] = $this->newtest_model->get_course_row($conditions);
        //echo_sql();
        //load the view
        $this->load->view('newtest/view_search_newtest', $data, false);
		}
...
/*payment gateway paytm integration END*/	
	...
	public function payment_status_paytm(){
	 ...
	 {
		$this->load->view('newtest/view_newtest_register');
	 }
	 public function speakeregister()
	 {
		$this->load->view('newtest/view_newtest_register_teacher');
	 }
	 public function newtestRegister()
	 {
		$password = $this->input->post('password',TRUE);
		$email = $this->input->post('user_name',TRUE);
		$name = $this->input->post('first_name',TRUE);
		$phone = $this->input->post('phone_number',TRUE);
		$newtest_id = $this->input->post('newtest',TRUE);
		if($newtest_id == "")
		{
		   echo "<script>alert('Please Select a Category from Dropdown'); window.location = '".base_url()."newtest/register'</script>";
		}
		$newtest_str = implode(",",$newtest_id);
		$ch = curl_init();  
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			{
				echo "<script>alert('Error Occured. Please Try Again'); window.location = '".base_url()."newtest'</script>";	
			}
			else
			{
				$dbe = $this->load->database('default', TRUE);
				$ip=$_SERVER['REMOTE_ADDR'];
				for($i=0;$i<count($newtest_id);$i++)
				{
					$sqq = "INSERT INTO `wl_newtest_register` (`emailid`, `newtest_id`, `ip`, `user_type`) values ('".$email."','".$newtest_id[$i]."','".$ip."', '1')";
					$que = $dbe->query($sqq); 
				}
				// $sqq = "INSERT INTO `wl_newtest_register` (`emailid`, `newtest_id`, `ip`, `user_type`) values ('".$email."','".$newtest_id."','".$ip."', '1')";
				// $que = $dbe->query($sqq); 
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			for($i=0;$i<count($newtest_id);$i++)
				{
					$sqq = "INSERT INTO `wl_newtest_register` (`emailid`, `newtest_id`, `ip`) values ('".$email."','".$newtest_id[$i]."','".$ip."')";
					$que = $dbe->query($sqq); 
				}
			// $sqq = "INSERT INTO `wl_newtest_register` (`emailid`, `newtest_id`, `ip`) values ('".$email."','".$newtest_id."','".$ip."')";
			// $que = $dbe->query($sqq); 
			echo "<script>alert('Registered Successfully. Proceed With Login'); window.location = '".base_url()."users/login'</script>";
	 ...
	 //TeacherRegistration
	 public function newtestRegisterTeacher()
	 {
		$password = $this->input->post('password',TRUE);
		$email = $this->input->post('user_name',TRUE);
		$name = $this->input->post('first_name',TRUE);
		...
		if($jsonOutput['message'] == "Invalid user")
		{
			...
			else
			{
				$dbe = $this->load->database('default', TRUE);
				$ip=$_SERVER['REMOTE_ADDR'];
				$sqq = "INSERT INTO `wl_newtest_Teacher` (`emailid`, `ip`, `user_type`) values ('".$email."','".$ip."', '1')";
				$que = $dbe->query($sqq); 
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			$dbe = $this->load->database('default', TRUE);
			$ip=$_SERVER['REMOTE_ADDR'];
			$sqq = "INSERT INTO `wl_newtest_Teacher` (`emailid`, `ip`) values ('".$email."','".$ip."')";
			$que = $dbe->query($sqq); 
			echo "<script>alert('Registered Successfully. Proceed With Login'); window.location = '".base_url()."teacher/login'</script>";
