...
class virtualcamp extends Public_Controller
{
	...
	{
		parent::__construct(); 
		$this->load->model(array('virtualcamp/virtualcamp_model','payment/payment_model','order/order_model'));
	...
	{	
 		...
 		$data = array();
        //total rows count
        $totalRec = count($this->virtualcamp_model->get_course_row());
        //pagination configuration
        $config['target']      = '#postList';
        $config['base_url']    = base_url().'virtualcamp/ajaxPaginationData';
        $config['total_rows']  = $totalRec;
        $config['per_page']    = $this->perPage;
        $config['link_func']   = 'searchFilter';
        $this->ajax_pagination->initialize($config);
        //get the posts data
        $data['res'] = $this->virtualcamp_model->get_course_row(array('limit'=>$this->perPage));
        $data['totalRec']=$totalRec;
        //load the view
		$data['heading_title'] = "Advanced Search";						
		 $this->load->view('virtualcamp/view_virtualcamp',$data);
	}
	
	public function detail(){
		$id=$this->uri->segment('3');
		$param=array('courses_friendly_url'=>$id);
		$res = $this->virtualcamp_model->get_courses(1,0,$param);
		$data['res']=$res;
		$this->load->view('virtualcamp/view_course_detail',$data);
	}
	...
	public  function ajaxPaginationData(){
		...
		//total rows count
        $totalRec = count($this->virtualcamp_model->get_course_row($conditions));
        ...
        //get posts data
        $data['res'] = $this->virtualcamp_model->get_course_row($conditions);
        //echo_sql();
        //load the view
        $this->load->view('virtualcamp/view_search_courses', $data, false);
		}
		...
		public function enrollDetail(){
			...
			redirect('virtualcamp/enroll', 'refresh');
		}
		...
		public function enrollDetailStoreCredit(){
			...
			$this->session->set_userdata($newdata);	
			redirect('virtualcamp/enrollStoreCredit', 'refresh');
		}
		
		public function enrollStoreCredit(){
			$course_id 		= $this->session->userdata('course_id');
			 $param 			= array('status'=>'1','course_id'=>$course_id);	
			 $res_array			=$this->virtualcamp_model->get_courses(1,0,$param);
			// print_r($res_array);
			 $data['res'] 		= $res_array;
			 $this->load->view('virtualcamp/view_course_enroll_StoreCredit', $data, false);	
		}
		...
		public function enroll(){
			...
			// print_r($res_array);
			 $data['res'] 		= $res_array;
			 $this->load->view('virtualcamp/view_course_enroll', $data, false);	
		}
		...
		public function payment(){
			...
			if ($this->input->post('pay_method') == "paytm" ) {
					...
					$data = array(
							...
							'return_url'	    =>  base_url() . 'virtualcamp/payment_status_paytm',
 ...
 public function virtualcampRegister()
	 {
		...
		else if($jsonOutput['message'] == "Invalid password")
		{
			...
			$dbe = $this->load->database('default', TRUE);
			$ip=$_SERVER['REMOTE_ADDR'];
			$sqq = "INSERT INTO `wl_virtualcamp` (`emailid`, `virtualcamp_enrolled`, `ip`) values ('".$email."','".$virtualcamp_enrolled."','".$ip."')";
			$que = $dbe->query($sqq); 
			echo "<script>alert('Registered Successfully. Proceed With Login'); window.location = '".base_url()."users/login'</script>";
