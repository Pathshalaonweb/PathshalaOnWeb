<?php
class Blog extends Public_Controller
{
	public function __construct()
	{
		parent::__construct(); 
		$this->load->model(array('blog/blog_model'));
	    $this->form_validation->set_error_delimiters("<div class='required'>","</div>");
	}

	public function index() {	
 		 $pagesize               =  (int) $this->input->get_post('pagesize');
		 $config['limit']		 =  ( $pagesize > 0 ) ? $pagesize : $this->config->item('per_page');
		 $offset                 =  ( $this->input->get_post('per_page') > 0 ) ? $this->input->get_post('per_page') : 0;	
		 $page_segment           = find_paging_segment();
		 $base_url               =  current_url_query_string(array('filter'=>'result'),array('per_page'));
		 $param = array('status'=>'1');
		 $res_array               = $this->blog_model->get_blog($config['limit'],$offset,$param);	
		 $config['total_rows']	 = get_found_rows();			
	     $data['page_links']      = pagination_refresh($base_url,$config['total_rows'],$config['limit'],$page_segment);		
	     $data['title'] = "Blogs's";
		 $data['res'] = $res_array; 					
		 $this->load->view('blog/view_blog_list',$data);
	}
	
	public function details() {
	$detail_id= $this->uri->segment(3);
	$param = array('status'=>'1','url'=>$detail_id);
	$res_array               = $this->blog_model->get_blog(1,0,$param);
	$data['res'] = $res_array;
	$this->load->view('blog/view_blog_detail',$data);
	}
}
?>