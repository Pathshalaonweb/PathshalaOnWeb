...
{	
	...
	{
		  $ci = CI();
		  $sql="SELECT category_id FROM tbl_categories WHERE parent_id=$catid $condtion ";
...
if ( ! function_exists('get_child_categories'))
{
   function get_child_categories($parent='0',$condtion="AND status='1'", $fields='SQL_CALC_FOUND_ROWS*')
   {
	     ...
	     $ci     = CI();
         $output        = array();
		 $sql           = "SELECT  $fields FROM tbl_categories WHERE parent_id=$parent $condtion  ";		 
		 $query         = $ci->db->query($sql);		
         $num_rows      =  $query->num_rows();	
...
if ( ! function_exists('get_parent_categories'))
{
   ...
   {	 
	     ...
	     $ci            = CI();
         $output        = array();
		 $sql           = "SELECT $fields FROM tbl_categories WHERE category_id=$category_id $condtion  ";		 		 
		 $query         = $ci->db->query($sql);		
         $num_rows      =  $query->num_rows();	
		...
		{ 
			...
			{ 
				 ...
				 while( $parent_id>0 )
				 {
					 $sql           = "SELECT $fields FROM tbl_categories WHERE category_id=$parent_id $condtion  ";		 		 
					 $query         = $ci->db->query($sql);		
					 $num_rows      =  $query->num_rows();	
...
if ( ! function_exists('get_nested_dropdown_menu'))
{
	function get_nested_dropdown_menu($parent,$selectId="",$pad="|__")
	{
		 ...
		 $var="";		 
		 $sql="SELECT * FROM tbl_categories WHERE parent_id=$parent AND status='1' ";		 
		 $query=$ci->db->query($sql);
		 $num_rows     =  $query->num_rows();
...
if ( ! function_exists('count_category'))
{
	...
	{		
		 ...
		 $sql="SELECT COUNT(category_id)  AS total_subcategories FROM tbl_categories WHERE $condtion ";		 
...
if ( ! function_exists('count_subject'))
{
	function count_subject($condtion='')
	{		
		 $ci = CI();
		 $condtion = "status !='2' ".$condtion;	 
		 $sql="SELECT COUNT(subject_id)  AS total_subject FROM tbl_subject WHERE $condtion ";		 
		 $query=$ci->db->query($sql)->row_array();
		 return  $query['total_subject'];
...
if ( ! function_exists('count_lession'))
{
	function count_lession($condtion='')
	{		
		 $ci = CI();
		 $condtion = "status !='2' ".$condtion;	 
		 $sql="SELECT COUNT(lession_id)  AS total_lession FROM tbl_lession WHERE $condtion ";		 
		 $query=$ci->db->query($sql)->row_array();
		 return  $query['total_lession'];
		 
	}
}
if ( ! function_exists('count_level'))
{
	function count_level($condtion='')
	{		
		 $ci = CI();
		 $condtion = "status !='2' ".$condtion;	 
		 $sql="SELECT COUNT(lession_id)  AS count_level FROM tbl_level WHERE $condtion ";		 
		 $query=$ci->db->query($sql)->row_array();
		 return  $query['count_level'];
				  ...
				  foreach( $query->result_array() as $row )
				  {
						...
						if ( has_child( $row['parent_id'] ) )
						{
								...
								if( $segment!='' && ( $row['category_id']==$segment ) )
								{
									...
									$link_cat[]='<li>'.$row['category_name'].'</li>';
