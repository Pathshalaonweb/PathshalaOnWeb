...
class Category extends Admin_Controller
{
	...
	public  function index()
	{
		...
		$data['heading_title']  =  ( $parent_id > 0 ) ? 'Subcourse' :  'Course';
		...
		$this->load->view('courses/view_category_list',$data);		
	...
	public function add()
	{
		...
		if( $parent_id!='' && $parent_id > 0 )
		{
			...
			$data['heading_title'] = 'Add Sub Course';
		...
		if($parent_id > 0)
		{
		 	$this->form_validation->set_rules('category_name','Category Name',"trim|required|max_length[100]|unique[tbl_categories.category_name='".$this->input->post('category_name')."' AND status!='2' AND parent_id='".$parent_id."']");
		}
		else
		{
			$this->form_validation->set_rules('category_name','Category Name',"trim|required|max_length[100]|unique[tbl_categories.category_name='".$this->input->post('category_name')."' AND status!='2' AND parent_id='0']");
		}
		...
		if($this->form_validation->run()===TRUE)
		{
		    ...
		    $this->category_model->safe_insert('tbl_categories',$posted_data,FALSE);	
		...
		$data['parent_id'] = $parent_id; 
		$this->load->view('courses/view_category_add',$data);		  
	...
	public function edit()
	{
		...
		$this->load->view('courses/view_category_edit',$data);				
