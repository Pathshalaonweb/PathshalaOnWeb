...
public function get_subject_list()
	{
	 ...
	 public function get_files($limit='10',$offset='0',$param=array())
	 {
		
		$category_id		=   @$param['category_id'];
		$status			    =   @$param['status'];	
		$filesid			=   @$param['productid'];
		...
		if($filesid!='')
		{
			$this->db->where("wlp.files_id  ","$filesid");
		}
		...
		if($keyword!='')
		{
			$this->db->where("(wlp.files_name LIKE '%".$keyword."%' OR wlp.files_code LIKE '%".$keyword."%' )");
		}
		...
		{
			$this->db->order_by('wlp.files_id ','desc');
		}
		
	    $this->db->group_by("wlp.files_id"); 	
		$this->db->limit($limit,$offset);
		$this->db->select('SQL_CALC_FOUND_ROWS wlp.*,wlpm.media,wlpm.media_type,wlpm.is_default',FALSE);
		$this->db->from('tbl_files as wlp');
		$this->db->where('wlp.status !=','2');
		$this->db->join('tbl_files_media AS wlpm','wlp.files_id=wlpm.files_id','left');
		$q=$this->db->get();
		//echo_sql();
	...
	{
		$data= array('files_id'=>$id);
		$query =	$this->db->get_where('tbl_files',$data);
		$result= $query->result_array();
		return $result = $result[0];
	} 
		  
	public function get_files_media($limit='4',$offset='0',$param=array())
    {		  
		 ...
		 {			
			...
			$this->db->limit($limit,$offset);
			$this->db->from('tbl_files_media');
			$this->db->where('files_id',$templateid);	
