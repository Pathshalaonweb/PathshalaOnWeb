...
{	
  ...
  if( ! function_exists('getMeta'))
  {
	function getMeta()
	{
		...
		{			
				  return array("meta_title"=>"LMS",
				  "meta_keyword"=>"LMS",
				   "meta_description"=>"LMS",
				   'dynamic_meta'=>TRUE 
				);
		}
	}	
...
if ( ! function_exists('category'))
{
	...
	function category($category_id)
	{
		...
		if( $category_id > 0 )
		{ 
			$res =  $CI->db->get_where($tbl,array('category_id'=>$category_id))->row();
  ...
  if ( ! function_exists('admin_currect_cans_count'))
{
	function admin_currect_cans_count($examid,$subjectid)
	{
	$ci = CI();		  
	$sql="SELECT * FROM `tbl_mock_result` where exam_id='".$examid."' and subject_id='".$subjectid."' and ans = user_ans ";		
...
if ( ! function_exists('admin_fetch_wans_count'))
{
	function admin_fetch_wans_count($examid,$subjectid)
	{
	$ci = CI();		  
	$sql="SELECT * FROM `tbl_mock_result` where exam_id='".$examid."' and subject_id='".$subjectid."' and ans != user_ans and user_ans != '' ";		
	$query=$ci->db->query($sql);		
	$currect_wns = $query->num_rows();
	return $currect_wns;
    //return count($currect_wns);
	}
	
}

if ( ! function_exists('admin_currect_tans_count'))
{
	function admin_currect_tans_count($examid)
	{
	$ci = CI();		  
	$sql="SELECT * FROM `tbl_mock_result` where exam_id='".$examid."'  and ans = user_ans  ";		
	$query=$ci->db->query($sql);		
	return $currect_tans = $query->num_rows();
    //return count($currect_tans);
	}
